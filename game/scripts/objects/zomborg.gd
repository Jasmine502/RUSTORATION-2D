...
func _ready():
	# Try finding player again in ready, in case it wasn't ready earlier
	if player == null: 
		player = get_tree().get_first_node_in_group("player")
	
	if player == null: 
		printerr("Zomborg Ready: Could not find node in group 'player'! Disabling physics.")
		set_physics_process(false) # Don't process movement if no player
		return # Exit _ready early
...
func _physics_process(_delta):
	# Added extra checks here for debugging
	if is_queued_for_deletion(): return # Don't process if dying

	if player == null or not is_instance_valid(player):
		# Try to find player again if lost (might happen in rare cases)
		player = get_tree().get_first_node_in_group("player")
		if player == null or not is_instance_valid(player):
			print("Zomborg Physics: Player reference lost or invalid, stopping.")
			velocity = Vector2.ZERO # Stop moving
			# Optionally disable processing completely: set_physics_process(false)
			return

	# --- DEBUG Print ---
	# print("Zomborg Pos: ", global_position, " | Player Pos: ", player.global_position) 

	var direction_to_player = (player.global_position - global_position).normalized()
	
	# --- DEBUG Print ---
	# print("Direction: ", direction_to_player, " | Speed: ", actual_speed)
...
func _on_hitbox_area_entered(area):
	if area.is_in_group("bullets"):
		var bullet_damage = 1
		if "damage" in area: bullet_damage = area.damage
...
func take_damage(amount: int):
	...
	if splat_sound_player != null and splat_sound_player.stream != null:
		splat_sound_player.pitch_scale = randf_range(0.9, 1.1); splat_sound_player.play()
	if sprite:
		sprite.modulate = Color(1, 0.5, 0.5)
		get_tree().create_timer(0.1).timeout.connect(_reset_color)
	if actual_health <= 0: die()

func _reset_color():
	if is_instance_valid(self) and sprite != null: sprite.modulate = Color(1, 1, 1)
...
func _try_drop_salvage():
	...
	if determined_drop_type == "unknown": printerr("Zomborg Error: Drop type undetermined!"); return
	var salvage_instance = salvage_scene.instantiate()
	if not salvage_instance is Area2D: printerr("Salvage instance not Area2D!"); salvage_instance.queue_free(); return
	...
	if salvage_container: salvage_container.call_deferred("add_child", salvage_instance)
	else: printerr("Salvage Drop WARN: 'salvage_container' node not found.")

func _randomize_sound_timer():
	if sound_timer:
		sound_timer.wait_time = randf_range(min_sound_interval, max_sound_interval); sound_timer.start()
...
func _on_sound_timer_timeout():
	if sound_player and zmb_sounds != null and not zmb_sounds.is_empty() and not sound_player.playing:
		...
		if random_sound is AudioStream:
			sound_player.stream = random_sound; sound_player.pitch_scale = randf_range(0.85, 1.15); sound_player.play()
		else: printerr("WARN: Zomborg sound element not AudioStream!")
	_randomize_sound_timer()
