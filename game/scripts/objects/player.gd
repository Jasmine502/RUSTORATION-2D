...
# --- SIGNALS ---
# Renamed signal and added max_health parameter
...
func _ready():
	...
	# Emit renamed signal with both values
	...
	# Null Checks and Setup... (rest is the same)
...
func _physics_process(delta):
	# ... (No changes needed in physics process itself) ...
	if current_health <= 0:
	...
	if shoot_timer != null and Input.is_action_pressed("shoot") and can_shoot:
		if shoot_sound_player != null and shoot_sound_player.stream != null:
			shoot_sound_player.pitch_scale = randf_range(0.9, 1.1); shoot_sound_player.play()
...
func _shoot():
	# ... (No changes needed in _shoot itself) ...
	...
	else: printerr("Bullet instance missing start() method."); bullet_instance.global_position = muzzle.global_position
...
func take_damage(amount: int):
	...
	if hurt_sound_player != null and hurt_sound_player.stream != null:
		hurt_sound_player.pitch_scale = randf_range(0.95, 1.05); hurt_sound_player.play()
...
func upgrade_health():
	...
	# Emit renamed signal with both values after health upgrade
	emit_signal("stats_updated", current_health, current_max_health) 
	
# Add this function anywhere inside the Player class in Player.gd
func get_current_max_health() -> int:
